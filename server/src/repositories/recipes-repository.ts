	...
	public async createNewRecipe(
		{
			...
			if (recipe)
				throw ApiError.BadRequest(`Recipe with title ${title} already exists in database.`);
	...
	public async initializeRecipes() {
		...
		fs.readFile(recipesDataFilePath, "utf8", (error, recipes) => {
			if (error) {
				...
				throw new Error("Unexpected error occurred. Could not read the necessary data for recipes.");
			}
